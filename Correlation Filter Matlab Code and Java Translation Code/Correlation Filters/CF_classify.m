...
function [classified, PSR] = CF_classify(dataIn, filters)
...
% Data import
% If needed, paste iteration through files here
% Assign the images to one cell matrix, 'filters'
...
for i = 1:numIn
    ...
    for j = 1:numFil
        currFil = filters{j};
        currFil = double(currFil);
        currFil = imresize(currFil, [32 32]);
        % Blurring for accuracy
        blur = fspecial('gaussian', [5 5], 0.2);
        currIn = imfilter(currIn, blur);
        % Calculating the correlation
        corr = ifft2(fft2(currIn).*conj(currFil));
